function solve() {
  ...
  function generate(e) {
    let input = JSON.parse(document.querySelector('text area').value);
    input.forEach(x => {
      ...
      p3.textContent = Number(x.dexFactor);
      td4.appendChild(p3);
      tr.appendChild(td4);
  ...
  function buy(e) {
    let checkboxes = Array.from(document.querySelectorAll('tbody input')).filter(c => c.cheked);
    let bought = [];
    let price = 0;
    let decoration = 0; 
